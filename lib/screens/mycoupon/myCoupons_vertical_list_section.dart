...
import 'package:infishare_client/blocs/my_coupon_bloc/bloc.dart';
...
class _UnusedCouponsVerticalListSectionState
  ...
  MyCouponBlocBloc _couponBlocBloc;

  @override
  void initState() {
    super.initState();
    _scrollController.addListener(_onScroll);
    _couponBlocBloc = BlocProvider.of<MyCouponBlocBloc>(context);
  }

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<MyCouponBlocBloc, MyCouponBlocState>(
      builder: (context, state) {
        if (state is TicketLoaded) {
          if (state.tickets.isEmpty) {
            return Center(
              child: ErrorPlaceHolder(
                AppLocalizations.of(context).translate('EmptyOrder'),
...
class _UsedCouponsVerticalListSectionState
  ...
  void _onScroll() {
    ...
    final currentScroll = _scrollController.position.pixels;
    if (maxScroll - currentScroll <= _threshold) {
      _couponBlocBloc.add(FetchUserCouponTicket(type: widget.type, used: true));
    }
  }
}
