...
class MyPassesBlocBloc extends Bloc<MyPassesBlocEvent, MyPassesBlocState> {
  final CouponRepository couponRepository;
  DocumentSnapshot _lastdoc;
  StreamSubscription _couponSub;

  MyPassesBlocBloc({this.couponRepository});

  @override
  MyPassesBlocState get initialState => TicketUninit();

  @override
  Stream<MyPassesBlocState> transformEvents(
    Stream<MyPassesBlocEvent> events,
    Stream<MyPassesBlocState> Function(MyPassesBlocEvent event) next,
  ) {
  ...
  @override
  Stream<MyPassesBlocState> mapEventToState(
    MyPassesBlocEvent event,
  ) async* {
    if (event is FetchUserPassesTicket) {
  ...
  Stream<MyPassesBlocState> _mapSelfRedeemTicketToState(
  ...
  Stream<MyPassesBlocState> _mapQRcodeRedeemTicketEventToState(
  ...
  Stream<MyPassesBlocState> _mapFetchUserVouchersToState(
      FetchUserPassesTicket event) async* {
    //* three conditions
  ...
  bool _hasReachMax(MyPassesBlocState state) {
